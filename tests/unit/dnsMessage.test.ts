...
describe('DNS response construction', () => {
  describe('answerFor', () => {
  ...
  describe('buildResponse', () => {
    it('answers an A query with the host IP, TTL 30', () => {
      ...
      expect(r.readUInt16BE(2) & 0x8000).toBe(0x8000);
      expect(r.readUInt16BE(2) & 0x0100).toBe(0x0100);
      expect(r.readUInt16BE(2) & 0x0080).toBe(0);
      expect(r.readUInt16BE(2) & 0x000f).toBe(0);
      ...
      expect(r.readUInt16BE(ansOff)).toBe(0xc00c);
      expect(r.readUInt16BE(ansOff + 2)).toBe(1);
      expect(r.readUInt16BE(ansOff + 4)).toBe(1);
      expect(r.readUInt32BE(ansOff + 6)).toBe(30);
      expect(r.readUInt16BE(ansOff + 10)).toBe(4);
    ...
    it('returns NOERROR with zero answers when there is no answer IP', () => {
      ...
      expect(r.readUInt16BE(6)).toBe(0);
      expect(r.readUInt16BE(2) & 0x000f).toBe(0);
      expect(r.length).toBe(parsed.query.questionEnd);
    });
