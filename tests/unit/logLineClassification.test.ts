...
describe('access-log line classification', () => {
  it('classifies TLS paths with port 443', () => {
    ...
    expect(classify(line({ pathId: 'passopen', serverName: 'open.example.com:443' }))[0].tag).toBe(
      'ALLOW OPEN',
    );
    expect(classify(line({ pathId: 'blocklist', serverName: 'blocked.example.com' }))[0].tag).toBe(
      'BLOCK LIST',
    );
  ...
  it('classifies http routes with port 80', () => {
    expect(
      classify(line({ pathId: 'http', authority: 'archive.ubuntu.com:80', routeName: 'matched' })),
    ).toEqual([
      { time: '2026-07-06T12:00:00', tag: 'ALLOW HTTP', domain: 'archive.ubuntu.com', port: 80 },
    ]);
    expect(
      classify(
        line({ pathId: 'http', authority: 'nope.example.com', routeName: 'default-deny' }),
      )[0].tag,
    ).toBe('BLOCK HTTP');
    expect(
    ...
    expect(
      classify(line({ pathId: 'http', authority: 'open.example.com', routeName: 'open' }))[0].tag,
    ).toBe('ALLOW OPEN');
  });
