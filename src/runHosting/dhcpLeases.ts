...
export function createLeaseTable(opts: LeaseTableOptions): LeaseTable {
  const start = opts.poolStart ?? 10;
  const end = opts.poolEnd ?? 209;
  const size = end - start + 1;
  const base = networkAddress(opts.hostIp, opts.netmask);
  const host = ipToInt(opts.hostIp);
  const now = opts.now ?? Date.now;
  const leases = new Map<number, Lease>();
  const declined = new Set<number>();
  const addressAt = (slot: number) => (base + start + slot) >>> 0;
  const existing = (identity: string) =>
    [...leases].find(([, l]) => l.identity === identity && l.expiresAt > now())?.[0] ?? null;
  const assign = (addr: number, identity: string) =>
  ...
  return {
    acquire(identity) {
      ...
      for (let i = 0; i < size; i++) {
        ...
        if (addr !== host && !declined.has(addr) && (!lease || lease.expiresAt <= now())) {
    ...
    request(identity, requested) {
      const addr = ipToInt(requested);
      const first = addressAt(0);
      const last = addressAt(size - 1);
      if (addr < first || addr > last || addr === host || declined.has(addr)) return 'nak';
      const lease = leases.get(addr);
      if (lease && lease.expiresAt > now() && lease.identity !== identity) return 'nak';
      assign(addr, identity);
      return 'ack';
    },
    release(identity) {
      const addr = existing(identity);
