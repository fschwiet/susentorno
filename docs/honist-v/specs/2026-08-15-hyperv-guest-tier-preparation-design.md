...
- **`apt upgrade -y`** stays. It is slow and it is a large cost in spec 2's end-to-end test, which makes it tempting to drop here — but it is a legitimate step in setting up a real user's guest, and removing it to speed up a test that does not yet exist would be the wrong reason.

  Spec 2 has a better lever anyway and should start from it rather than rediscover it: **apply the upgrade once when building the golden VHDX** (`package_upgrade: true` in the seed, or an autoinstall `late-command`), so every differencing guest boots already current and the per-run upgrade finds nothing to do. This decays rather than eliminating the cost — the golden image is rebuilt only when its seed inputs change, so an old image has a correspondingly long upgrade to apply — and bounding it would mean adding a maximum image age to the stamp check. Note also what this does *not* help: `03-install-tools.sh`'s three `curl | bash` agent installers and `pnpm runtime set node latest -g` re-fetch from upstream on every run regardless of image contents, and are likely to dominate once the upgrade is pre-applied.

  A related risk spec 2 introduces and must handle: Ubuntu's `apt-daily` and `unattended-upgrades` timers fire shortly after boot and hold the dpkg lock. Today's harness never trips this because its guest tests run only `nn-configure-network.sh` and the post-scripts, never `01`–`03`. An end-to-end test running real `apt install` on a freshly booted guest sits squarely in that window, so the golden image should mask those timers the way it already masks `systemd-networkd`.
