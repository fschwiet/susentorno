...
- **No new PowerShell test.** Nothing in the suite drives `05-github-auth.ps1` behaviorally today (only a static content check in `tests/unit/templates.test.ts`, which this change doesn't affect) — there's no `gh`/Windows harness to exercise the exit-code path against, so this is a known, accepted gap rather than one this fix invents a mock to paper over.
