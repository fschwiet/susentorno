...
```powershell
New-VMSwitch -Name "susentorno-internal" -SwitchType Internal
...
This step is invoked from inside an environment (the script is generated by `susentorno init` — see [setup-environment.md](setup-environment.md)), but its effect is scoped to the host's Internal-switch adapter, not to any one environment. Do it once, using whichever environment you set up first; later environments don't need to repeat it unless the adapter or its IP changes.
